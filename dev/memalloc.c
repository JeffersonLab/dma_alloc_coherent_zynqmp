...
static long
memalloc_ioctl(struct file *fd, unsigned int cmd, unsigned long arg)
{
  struct ioctl_arg_t ioctl_arg;
  long status;

  status =
    copy_from_user(&ioctl_arg, (void __user *) arg, sizeof(ioctl_arg_t));
  if (status != 0)
    {
  ...
  switch (cmd)
      ...
      if (status != 0)
	{
      ...
      status =
	copy_to_user((ioctl_arg_t *) arg, &ioctl_arg, sizeof(ioctl_arg_t));
      if (status != 0)
	{
    ...
    case MEMALLOC_ACTIVATE_BUFFER_CMD:
      printk(KERN_ERR "DEBUG: Module fops->ioctl: ACTIVATE_BUFFER (id %d).\n",
	     ioctl_arg.buffer_id);
      if (ioctl_arg.buffer_id > MEMALLOC_BUFFER_MAX_NUMBER
	  || buffer_info[ioctl_arg.buffer_id].active == 0)
	{
...
static int
memalloc_mmap(struct file *fd, struct vm_area_struct *vma)
{
  ...
  return dma_mmap_coherent(NULL, vma,
			   buffer_info[active_buffer_id].kernel_address,
			   buffer_info[active_buffer_id].handle,
			   vma->vm_end - vma->vm_start);
}

static int
memalloc_release(struct inode *in, struct file *fd)
{
...
static int
memalloc_open(struct inode *ino, struct file *file)
{
...
static int
reserve_buffer(ioctl_arg_t * ioctl_arg)
{
  int id;
  size_t size;
  void *vaddr;
  ...
  if (id < MEMALLOC_BUFFER_MAX_NUMBER)
      ...
      if (status != 0)
	{
      ...
      vaddr =
	dma_alloc_coherent(interface.device_p, size, &paddr, GFP_KERNEL);
#else
...
static int
release_buffer(ioctl_arg_t * ioctl_arg)
{
  ...
  dma_free_coherent(NULL, buffer_info[id].size,
		    buffer_info[id].kernel_address, buffer_info[id].handle);
  return (0);
}

static int
get_physical_address(ioctl_arg_t * ioctl_arg)
{
  int id = ioctl_arg->buffer_id;
...
cleanup(void)
{
  ...
  for (i = 0; i < MEMALLOC_BUFFER_MAX_NUMBER; i++)
	...
	{
	  dma_free_coherent(NULL, buffer_info[i].size,
			    buffer_info[i].kernel_address,
			    buffer_info[i].handle);
...
memalloc_init(void)
{
  ...
  if (rc)
    {
      printk(KERN_ERR
	     "ERROR: Initialize and add the character device: FAILED.\n");
  ...
  interface.device_p =
    device_create(interface.class_p, NULL, interface.dev_node, NULL,
		  DEVICE_NAME);
...
MODULE_DESCRIPTION
  ("Create a buffer and return physical and virtual address, for DMA userspace driver. Thanks to Massimiliano Giacometti.");
MODULE_LICENSE("GPL");
